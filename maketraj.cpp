...
//! Calculate the legs
void makeTraj::calculateTrajectory( double& totalDeltaV )
{
...
void makeTraj::intermediatePoints( double maximumTimeStep,
...
void makeTraj::maneuvers( std::vector < Eigen::Vector3d >& positionVector,
                            std::vector < double >& timeVector,
...
void makeTraj::planetaryOrbits( double maximumTimeStep,
...
void makeTraj::planetaryEncounters( std::vector < Eigen::Vector3d >& positionVector,
                          std::vector < double >& timeVector )
{
...
void makeTraj::updateVariableVector( const Eigen::VectorXd& trajectoryVariableVector )
{
...
void makeTraj::prepareVelocityAndPositionVectors( )
{
...
int makeTraj::checkTrajectoryVariableVectorSize( )
{
...
void makeTraj::extractEphemeris( )
{
    ...
    for ( int counter = 0; counter < numberOfLegs_ ; counter++ )
    {
        ...
        if(loco=='1'||loco=='2'||loco=='3'||loco=='4'||loco=='5'||loco=='6'){
            ...
            switch(locoint){
            case(1):{
                planetPositionVector_[ counter ] = getLibrationState ("Em", 1, time, true).segment(0,3);
                planetVelocityVector_[ counter ] =  getLibrationState ("Em", 1, time, true).segment(3,3);
                break;
            }
            case(2):{
                planetPositionVector_[ counter ] = getLibrationState ("Em", 2, time, true).segment(0,3);
                planetVelocityVector_[ counter ] =  getLibrationState ("Em", 2, time, true).segment(3,3);
                break;
            }
            case(3):{
            ...
            case(5):{
                planetPositionVector_[ counter ] = getLibrationState ("SE", 2, time, false).segment(0,3);
                planetVelocityVector_[ counter ] = getLibrationState ("SE", 2, time, false).segment(3,3);
                break;
            }
            case(6):{
                planetPositionVector_[ counter ] = getLibrationState ("SM", 1, time, false).segment(0,3);
                planetVelocityVector_[ counter ] = getLibrationState ("SM", 1, time, false).segment(3,3);
                break;
            }
...
void makeTraj::getLaunchConditions( Eigen::Vector3d& departureBodyPosition,
