...
enum B_LegTypeTransfer
{
    B_MGA_Departure = 1,
    B_MGA_Swingby,
    B_MGA1DsmPosition_Departure,
    B_MGA1DsmPosition_Swingby,
    B_MGA1DsmVelocity_Departure,
    B_MGA1DsmVelocity_Swingby,
    B_Capture
};
...
public:
     ...
     */
    B_makeTraj( const double numberOfLegs,
                const std::vector< B_LegTypeTransfer >& legTypeVector,
                std::string flyby_seq,
                ...
                const std::map<double, Eigen::VectorXd> specificOrbit,
                const double orbitFraction,
                ...
                const bool includeArrivalDeltaV = true )
    {
        ...
        // Set Analysis B parameters needed for orbit modelling
        specificOrbit_ = specificOrbit;
        orbitFraction_ = orbitFraction;
        stayTime_ = stayTime;

        // Set the number of legs.
        numberOfLegs_ = numberOfLegs;

        //Set the flyb sequence vector
        flybyseq_ = flyby_seq;
...
private:
    ...
    //! Parameters needed in Analysis B for orbit modelling.
    /*!
     * Parameters needed in Analysis B for orbit modelling.
     */
    std::map<double, Eigen::VectorXd> specificOrbit_;
    int orbitID_;
    double orbitFraction_;
    double stayTime_;
     ...
     */
    std::vector< B_LegTypeTransfer > legTypeVector_;
